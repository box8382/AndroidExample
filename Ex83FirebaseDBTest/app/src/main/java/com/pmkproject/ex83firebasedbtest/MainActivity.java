package com.pmkproject.ex83firebasedbtest;
...
public class MainActivity extends AppCompatActivity {
    ...
    //처음 시작은 Firebase console에서
    //[프로젝트 만들기 or 프로젝트 추가]를 통해
    //작업순서[workflow]대로 이 프로젝트와 firebase를 연결하기
    EditText et;
    TextView tv;
    ...
    public void clickSave(View view) {
        ...
        //2. 저장시킬 노드 참조객체 가져오기!
        DatabaseReference rootRef=firebaseDatabas.getReference(); //파라미터에 아무것도 안주면 최상위 노드
...
//        //별도의 읽어오기 버튼이 없음..
//        //실기간 DB인 만큼
//        //데이터베이스 변경이 발생하면 이에 반응하는 리스너를 통해 실시간 DB를 읽어옴
...
//        });
        ...
        //위 작업은 값이 변경되면 기존에 있는값을 덮어씌기해서 전에값은 사라진다
        //2) 누적하여 값 저장
...
//            @Override
//            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
//                //String data=dataSnapshot.getValue(String.class);//에러
...
//                StringBuffer buffer=new StringBuffer();
//                for(DataSnapshot snapshot:dataSnapshot.getChildren()){
//                    buffer.append(snapshot.getValue(String.class));
//                }
//                tv.setText(buffer.toString());
//
//            }
...
//
//        //'member'라는 이름의 노드(node)에 리스너 추가
...
//        });
        ...
        //5) 나만의 클래스(PersonVO) 객체를 만들어서 한방에 멤버값들 저장하기
        ...
        //'person'라는 노드에 리스너 붙이기
        personRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                //persons 노드는 여러개의 자식노드가 있으므로
                StringBuffer buffer=new StringBuffer();
                for(DataSnapshot snapshot: dataSnapshot.getChildren()){

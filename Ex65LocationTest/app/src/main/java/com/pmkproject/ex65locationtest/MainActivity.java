package com.pmkproject.ex65locationtest;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvProviders = findViewById(R.id.providers);
        ...
        //1. gps : 가장 높은 정확도, 무료, 실내에서 사용불가
        //2. network(wifi,3g, 4g, lte, 5g) : 중간정도의 정확도, 유료라고 보는게 맞음.
        //3. passive : 다른 앱의 마지막 위치정보를 사용. 정확도 가장 낮음

        //디바이스에서 사용 가능한 위치 정보 제공자를 확인
        ...
        //위치정보 제공자 중에서 최고의 제공자 판별
        //최고의 제공자를 판별하기위한 기준(criteria)
        ...
        //Best 위치정보를 얻으려면 퍼미션필요함!!!!!
        String bestProvider = locationManager.getBestProvider(criteria, true);
        tvBestProvider.setText(bestProvider);

        //마시멜로우(api23버전)부터 동적 퍼미션 필요 : 앱을 다운로드할 때 뿐만아니라, 사용할 때도 퍼미션을 체크 하는 방식
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            //퍼미션이 허가되어 있지 않은지?                                            허가상태
            if (checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) { //허가되어있지 않은가?
    ...
    public void clickBtn(View view) {
        ...
        //매니저를 이용해서 위치정보 얻어오기
        if (locationManager.isProviderEnabled("gps")) { //gps가 켜져있을때만!
        ...
        if(location==null){
            tvMyLocation.setText("위치를 찾을 수 없습니다");
        }else{
            //위치, 경도 얻어오기
            ...
            //왕십리역 좌표: 37.561169, 127.037752

            //내 위치(latitude, longitude)와 왕십리역 사이의 실제 거리(m)
            float[] result=new float[3];//거리계산 결과를 저장할 배열 객체
            Location.distanceBetween(latitude, longitude,37.561169,127.037752, result);

            //result[0]에 두 좌표 사이의 m거리가 계산되어 저장되어 있음
            if(result[0]<50){ //두 좌표거리가 50m 이내 인가?
                if(isEnter==false){

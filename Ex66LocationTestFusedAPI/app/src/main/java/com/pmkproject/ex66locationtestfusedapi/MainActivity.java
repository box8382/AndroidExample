package com.pmkproject.ex66locationtestfusedapi;
...
public class MainActivity extends AppCompatActivity {
    ...
    GoogleApiClient googleApiClient; //위치정보관리 객체[LocationManager 역활]
    FusedLocationProviderClient providerClient; //위치정보제공자 객체[알아서 적절한 제공자 선택] gps,network,passive
    ...
    public void clickBtn(View view) {
        ...
        //위치정보 객체를 만들어주는 건축가 객체 생성
        GoogleApiClient.Builder builder=new GoogleApiClient.Builder(this);
        //건축가에게 FusedAPI 사용 인증키 지정
        builder.addApi(LocationServices.API);
        //건축가에게 위치정보 탐색연결 성공 리스너 (위치정보 탐색연결 성공 콜백리스너 객체)
        builder.addConnectionCallbacks(new GoogleApiClient.ConnectionCallbacks() {
            ...
            public void onConnected(@Nullable Bundle bundle) {
                ...
                locationRequest.setInterval(2000); //2초에 한번씩 위치정보 갱신
        ...
        builder.addOnConnectionFailedListener(new GoogleApiClient.OnConnectionFailedListener() {
            ...
            public void onConnectionFailed(@NonNull ConnectionResult connectionResult) {
                Toast.makeText(MainActivity.this, "위치정보 연결에 실패했습니다", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        //위치정보관리 객체 에게 위치정보연결 시도
        googleApiClient.connect();

        //위치정보에 실시간 위치정보 취득을 위해 위치정보제공자 소환

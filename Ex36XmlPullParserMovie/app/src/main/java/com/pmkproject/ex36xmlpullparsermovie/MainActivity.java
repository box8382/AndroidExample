package com.pmkproject.ex36xmlpullparsermovie;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void clickBtn(View view) {
        new Thread(){
            ...
            public void run() {
                //영화진흥위원회 openAPI를 통해
                //일일 박스오피스 정보를 가진 xml 문서를
                //읽어와서 분석하여 listview에 보여주기
                items.clear();

                Date date=new Date();//현재 날짜와 시간을 가진 객체
                date.setTime(date.getTime()-(1000*60*60*24) ); //1일전시간으로 바꾸기
                ...
                //위 xml 문서의 주소(address)에 스트림을 연결하여 데이터를 읽어오기
